...
export default function IntroScreen({ onComplete }: Props) {
  ...
  return (
    <motion.div
      ...
      className="fixed inset-0 bg-neutral-900 z-[100] overflow-y-auto p-4 sm:p-8"
    >
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 0.5, duration: 1 }}
        className="w-full max-w-3xl mx-auto bg-[#f4f1ea] text-neutral-900 p-6 sm:p-10 md:p-12 shadow-2xl relative font-serif shrink-0 mt-8 sm:mt-16 mb-8"
      >
        {/* News Header */}
        <div className="border-b-4 border-neutral-900 pb-4 mb-6 sm:mb-8 text-center">
          <h1 className="text-4xl sm:text-5xl md:text-6xl font-black tracking-tighter mb-4 text-neutral-900">
            THE DAILY CHRONICLE
          </h1>
          <div className="flex justify-between text-xs sm:text-sm font-bold text-neutral-600 uppercase border-y-2 border-neutral-900 py-1.5 px-2">
            <span>Vol. 1</span>
        ...
        >
          ...
          <p className="text-base sm:text-lg md:text-xl font-medium leading-relaxed text-neutral-800 text-justify">
        ...
        >
          ...
          <p className="text-base sm:text-lg md:text-xl font-medium leading-relaxed text-neutral-800 text-justify">
            정부는 즉각 비상사태를 선포하였으며, 현재 대한민국은 극도로 통제된 안전 구역인 <strong className="bg-neutral-900 text-white px-1">세이프 존</strong>과 통제 불능의 비안전 구역 <strong className="bg-red-700 text-white px-1">데드 존</strong>으로 양분되어 생존자들의 험난한 사투가 이어지고 있다.
          </p>
        </motion.div>
